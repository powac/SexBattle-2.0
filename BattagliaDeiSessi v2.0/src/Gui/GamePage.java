...
public class GamePage {
    ...
    private static int redAreaBomb = 255;                                            // red component 0...255
    private static int greenAreaBomb = 70;                                            // green component 0...255
    private static int blueAreaBomb = 70;                                            // blue component 0...255
    private static int alphaAreaBomb = 255;                                          // alpha (transparency) component 0...255
    private static int colorRedAreaBomb = (alphaAreaBomb << 24) | (redAreaBomb << 16) | (greenAreaBomb << 8) | blueAreaBomb;
    ...
    private static int alphaWhite = 0;                                          // alpha (transparency) component 0...255
    ...
    public synchronized void startGamePage(int personeIniziali, int anniTotali, int a, int b, int c,
        ...
        VBox vbButton = new VBox(chartBehaviour, atomicBomb, startStop, terminaGioco);
        ...
        HBox hInfo = new HBox(anniTrascorsi, numeroMaschi, numeroFemmine);
        ...
        javafx.application.Platform.runLater(() -> {
            ...
            threadAggiornaPixel = new Thread(() -> {
                ...
                for(int i = 0; i < personeIniziali; i++) {
                    generazioneIniziale.add(new People());
                }
                Population popolazioneUniverso = new Population(generazioneIniziale);
                updateInfoGUI(popolazioneUniverso, anniTrascorsi, numeroMaschi, numeroFemmine,
                        numeroMorigerati, numeroAvventurieri, numeroPrudenti, numeroSpregiudicate);
                        ...
                        public void handle(javafx.scene.input.MouseEvent event) {
                            if(eventoClick) {
                                for(int xMatrice = 0 ; xMatrice < originalImage.getWidth(); xMatrice++) {
                                    for(int yMatrice = 0 ; yMatrice < originalImage.getHeight(); yMatrice++) {
                                        if(matriceBomba[xMatrice][yMatrice] != -1) {
                                            drawAtomicArea(originalImage, imView, wr, yMatrice, xMatrice, colorWhiteAreaBomb);
                ...
                for(int i = 0; i < anniTotali; i++) {
                    ...
                    if(Thread.currentThread().isInterrupted() || terminaPartita) {
                        break;
                    }
    ...
    private synchronized void updateInfoGUI(Population popolazioneUniverso,
        ...
        numeroFemmine.setText("Femmine: " + sexualityAnalyzer.sexualityCounter().get(PeopleFactory.Sexuality.Femmina) + " ( " +
                percentualiFemminaFloat + " % del totale)");
