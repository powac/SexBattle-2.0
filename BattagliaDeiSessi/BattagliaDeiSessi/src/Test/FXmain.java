...
import Populations.FxAnalyzer.AgeChart;
import Populations.FxAnalyzer.BehaviourChart;
import Populations.FxAnalyzer.HpChart;
import Populations.FxAnalyzer.SexualityChart;
...
public class FXmain extends Application
{
    ...
    public synchronized void start(Stage primaryStage) throws ExecutionException, InterruptedException
    {
        ...
        /*
        GamePage provaaa=new GamePage(primaryStage);
        try {
            provaaa.startGamePage(0,0,0,0,0, new AverageGainStrategy());
        } catch (IOException e) {
            e.printStackTrace();
        }
        */
        ...
        Runnable task = () ->
        {
            Population popolazioneUniversoTornata = null;
            try {
                popolazioneUniversoTornata = inizio();
            ...
            //AgeChart ac = new AgeChart();
            //HpChart hc = new HpChart();
            //SexualityChart sc = new SexualityChart();

            Population finalPopolazioneUniversoTornata = popolazioneUniversoTornata;

            javafx.application.Platform.runLater(()->
            {
                primaryStage.setTitle("Chart");
                primaryStage.setScene(bc.setData(finalPopolazioneUniversoTornata));
                //primaryStage.setScene(ac.setData(popolazioneUniversoTornata));
                //primaryStage.setScene(hc.setData(popolazioneUniversoTornata));
                //primaryStage.setScene(sc.setData(popolazioneUniversoTornata));
    ...
    public synchronized Population inizio() throws ExecutionException, InterruptedException
    {
        ...
        for(int i=0;i<31;i++)
        {
            personeIniziali.add(new FirstPeoples());
        }
        int a=15;
        int b=20;
        int c=3;
        ...
        for(int i=0;i<1;i++)
        {
            ...
            System.out.println("\nMorigerati: "+behaviourAnalyzer.behaviourCounter().get(PeopleFactory.Behaviour.Morigerato) +" ovvero il "+
                    behaviourAnalyzer.behaviourCounterX100().get(PeopleFactory.Behaviour.Morigerato)+" %"+
            "       Punteggio in ottavi: "+ (((behaviourAnalyzer.behaviourCounter().get(PeopleFactory.Behaviour.Morigerato)*100)/sexualityAnalyzer.sexualityCounter().get(PeopleFactory.Sexuality.Maschio))*800)/100+" / 800"+
                    "   Deve essere 500/800");

            System.out.println("Avventurieri: "+behaviourAnalyzer.behaviourCounter().get(PeopleFactory.Behaviour.Avventuriero) +" ovvero il "+
                    behaviourAnalyzer.behaviourCounterX100().get(PeopleFactory.Behaviour.Avventuriero)+" %"+
                    "       Punteggio in ottavi: "+(((behaviourAnalyzer.behaviourCounter().get(PeopleFactory.Behaviour.Avventuriero)*100)/sexualityAnalyzer.sexualityCounter().get(PeopleFactory.Sexuality.Maschio))*800)/100+" / 800"+
                    "   Deve essere 300/800");
            ...
            System.out.println("Prudenti: "+behaviourAnalyzer.behaviourCounter().get(PeopleFactory.Behaviour.Prudente) +" ovvero il "+
                    ...
                    "       Punteggio in sesti: "+(((behaviourAnalyzer.behaviourCounter().get(PeopleFactory.Behaviour.Prudente)*100)/sexualityAnalyzer.sexualityCounter().get(PeopleFactory.Sexuality.Femmina))*600)/100+" / 600"+
            "   Deve essere 500/600");

            System.out.println("Spregiudicate: "+behaviourAnalyzer.behaviourCounter().get(PeopleFactory.Behaviour.Spregiudicata) +" ovvero il "+
                    behaviourAnalyzer.behaviourCounterX100().get(PeopleFactory.Behaviour.Spregiudicata)+" %"+
                    "       Punteggio in sesti: "+(((behaviourAnalyzer.behaviourCounter().get(PeopleFactory.Behaviour.Spregiudicata)*100)/sexualityAnalyzer.sexualityCounter().get(PeopleFactory.Sexuality.Femmina))*600)/100+" / 600"+
                    "   Deve essere 100/600");
